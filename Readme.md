## Two STM32F103 Midi USB Interface Units based on Midiklik4x4
Two Midi Xpoint switch boxes were made - a 5x5 and a 2x3. These are replacements for a 4x4 Xpoint Midi diode-based switch constructed in the early 1990's. They use a software Midi router and switch based on [**MidiKlik4x4**](https://github.com/TheKikGen/USBMidiKliK4x4/tree/master) - Midiklik is an unusual MidiUSB interface because of the extensive range and number of [**routing, filters and pipes**](https://github.com/TheKikGen/USBMidiKliK4x4/blob/master/UMK-4X4-SYSEX-IPL.TXT), and other configuration options, all changeable via its set of internal SysEx commands.

The much smaller 2x3 midi switch box makes use of a previously built [**Pico Macro Pad 2.8 inch Touch LCD**](https://github.com/TobiasVanDyk/Pico-MCU-from-Raspberry-Pi/tree/main/TouchMacroPadPico). It re-used the top LCD+Pico Lid and adapted the bottom to fit a single STM32F103 beneath the Pico and the Waveshare 2.8inch LCD. It connects via two 4pin JST 2.54mm sockets (TX2, RX2, TX3, RX3, 3v3 and Gnd) mounted at the back on a long piece of stripboard, together with an external reset button for the STM32, to two external Midi Input and Output units. As an alternative two TRRS 4-pole sockets could also be considered. It is expandable to the same size as the larger 5x5 switch unit that uses two STM32's if another 4pin socket is added, and the 2nd STM32F103 connected via i2c. The STM32 TX1 and RX1 are connected to the Pico's RX0 and TX0. The same Pico and STM32 software used for the earlier 5x5 unit were loaded on the 2x3 unit, and a socket to expand it to 5x5 will be added in the future. The software used is in the PicoKlik33 folder (as PicoKlik2x3a.zip), and is compiled for the Waveshare ST7789 LCD. The four photos below show the construction of the smaller box and one 6N137 Midi I/O unit (photo and stripboard layout). The 3D-Case STL files are in the STL folder as MidiUSB3x3Bottom5.stl, MidiUSB3x3Lid3b.stl, MidiUSB3x3Clamp2.stl (print the clamp as a mirror), and Midi3x3IO.stl. With only two 4pin JST sockets it is strictly speaking only a 2x2 unit but the STM32 Tx1 connection can be easily used as a third Midi output. There are also four USB Midi Input/Output cables/channels to use in the smaller unit - refer to the first photo below, where the USB Midi channels are the four rows. The third photo used the PicoKlik2x3 in offline mode (USB cable only used as PSU), and the Midi IO selection switching is then done from a different page.

<p align="left">
<img src="images/midi2a.jpg" height="180" /> 
<img src="images/midi6.jpg" height="180" /> 
<img src="images/midi12.jpg" height="180" /> 
<img src="images/midi2x3io.jpg" height="180" /> 
</p> 
...
```
The modified files are inside the folder PicoKlik33 as USBMidiKliK4x4.zip.
...
XPoint Switch Colours: Black indicates no routing connection between input Jack A-E and output Jack 1-5. Pressing on any of the 25 crosspoints will toggle it on/off, and the colour will change to Red when an Input-Output Jack-pair is connected. Green indicates that a Slot is attached to an input Jack and the second character on the switch button is then the slot number 1-8 instead of the output number 1-5. When the [Sl] button is active (i.e. Green), pressing any of the attached buttons in a row will toggle the slot on/off. The [Pp] and [Rt] button functions are explained in the comments of the source code file [**PicoKlik33.ino**](https://github.com/TobiasVanDyk/STM32F103-Midi-USB-Interface/blob/main/PicoKlik33/PicoKlik33.ino) - note that the three buttons [Rt], [Sl], and [Pp] are green when active and blue when not. Combinations of these three buttons are also used to for example set/unset a Pipe Bypass.
...
<img src="images/picoklik2a.jpg" height="180" /> 
</p>
