...
Two namely a 5x5 and a 2x3, XPoint Midi switch boxes were made. These are replacements for a 4x4 Xpoint Midi diode-based switch constructed in the early 1990's. They use by using a software Midi router and switch based on [**MidiKlik4x4**](https://github.com/TheKikGen/USBMidiKliK4x4/tree/master) - Midiklik is an unusual MidiUSB interface because of the extensive range and number of [**routing, filters and pipes**](https://github.com/TheKikGen/USBMidiKliK4x4/blob/master/UMK-4X4-SYSEX-IPL.TXT) and other configuration options, changeable via its set of internal SysEx commands.

The much smaller 2x3 midi switch box makes use of a previously built [Pico Macro Touch Interface](https://github.com/TobiasVanDyk/Pico-MCU-from-Raspberry-Pi/tree/main/TouchMacroPadPico). It re-used the top LCD+Pico Lid and adapted the bottom to fit a single STM32F103 beneath the Pico and the 2.8inch LCD. It connects via two 4pin JST 2.54mm sockets (Tx2, Rx2, Tx3, Rx3, 3v3 and Gnd) mounted at the back on a long piece of stripboard, together with an external reset button for the STM32, to two external Midi Input and Output units. It is expandable to the same size as the 5x5 switch unit that uses two STM32's if another 4pin socket is added and the 2nd STM32 connected via i2c. The STM32 Tx1 and Rx1 are connected to the Pico's Rx0 and Tx0. The same Pico and STM32 software used for the earlier 5x5 unit were loaded on the 2x3 unit, as a socket to expand it to 5x5 will be added at a later stage. The two photos below show the construction of the smaller box, and the 3D-Case STL files are in the STL folder as MidiUSB3x3Bottom5.stl, MidiUSB3x3Lid3b.stl and MidiUSB3x3Clamp2.stl (print the clamp as a mirror). With only two 4pin JST sockets it is strictly speaking only a 2x2 unit but the STM32 Tx1 connection can be easily used as a third Midi output.
