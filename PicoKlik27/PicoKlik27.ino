...
byte SlotPipeId1[8] = { 0,0,0,0,0,0,0,0 }; // 1st Pipe in each Slot 1-8 Pipe ID such as 00, 01 etc Empty = 0x7F
byte SlotPipeId2[8] = { 0,0,0,0,0,0,0,0 }; // 2nd Pipe in each Slot 1-8 Pipe ID such as 00, 01 etc Empty = 0x7F
byte SlotPipeBp1[8] = { 0,0,0,0,0,0,0,0 }; // First Pipe in each Slot 1-8 Bypass = 1 Execute = 0
...
bool ae15[2][5][5] = {};           // 0=IThru=Layout1 1=Midi=Layout2 JacksRouting Ports 1-5 = ae15[0-4][0-4] i.e. exclude Port 0
...
bool DoSync = true;
int Sync = 0;
bool ShowSysX = false;     // Show received SysEx
...
{ uint8_t p, n, i, JackOut1, JackOut2, SlotIThru, SlotMidi, JackIn; 
  ...
  for (JackIn=1; JackIn<6; JackIn++)                                        // = arr[7] to arr[11]
      {JackOut1  = Sync1[JackIn+6];                                         // JackOut 1-5 bits in each arr[7] - [11]
       JackOut2  = Sync1[JackIn+12];                                        // JackOut 1-5 bits in each arr[12] - [17]
       SlotIThru = Sync1[JackIn+18];                                        // SlotIThru 0,1-8 in each arr[18] - [23]
       SlotMidi  = Sync1[JackIn+24];                                        // SlotMidi 0,1-8 in each arr[24] - [29]                           
...
{ uint8_t i;
  ...
  for (i=0; i<8; i++)                                        // First 2 Pipes in exach Slot 1-8                                     
      {SlotPipeId1[i] = Sync2[8+i];   
       SlotPipeId2[i] = Sync2[16+i]; }   
}
...
{ uint8_t p, n, i, JackOut1, JackOut2, SlotIThru, SlotMidi, JackIn; 
  ...
  if (ShowSysX) 
     {n = 0; uint8_t m = 0; uint8_t a = 11;  uint8_t b;                     
      while (n<a) { b = arr[n+4];                                   
...
{ int i;
  ...
  if (DoSync) { if ((NowMillis - LastMillis) >= SyncPeriod)                
                   {Sync++; 
                    ...
                    if (Sync==3) { MIDI1.sendSysEx(sizeof(iThruConf), iThruConf, true); SyncPeriod = 11000; }                                                                       
                    if (Sync==4) { DoSync2(); DoSync = false; status("Sync Completed");                     }  } 
               }  // if (DoSync)
...
{ byte b, n, i;
  ...
  if (c>0) SlotPipeBp1[b-1] = 1;     // NotSynced Use Column 0,1 = 1 2,3,4 = 0 
      else SlotPipeBp1[b-1] = 0;     // Toggle ByPass On/Off - can also b = !b; b ^= 1;
  SetPipeBP[7] = b;                  // Get row pressed 1-5 then slot 1-8
  SetPipeBP[8] = pipeBP;             // Pipe Index Position 0-7 
  SetPipeBP[9] = SlotPipeBp1[b-1];   // On/Off 0/1

  MIDI1.sendSysEx(11, SetPipeBP, true);

  //  /*
  n = 4; int m = 0; int a = 11;             // n=4; starts at 11               
  while (n<a) { b = SetPipeBP[n];                                  
                Int2HexStr[n+m]   = digits[(b >> 4) & 0x0F];   
                Int2HexStr[n+m+1] = digits[b & 0x0F]; n++; m=m+3; }    
  status((char *)Int2HexStr); 
  // */
...
{ int r, c, OnColour = Red;
  for (r=0; r<5; r++) { if ( p==0 && iThru15[r] == false ) OnColour = Magenta; else OnColour = Red; // Takes a resync if iThru switched on
                        for (c=0; c<5; c++) if (ae08[p][r]==0) { if (ae15[p][r][c]) keyColor12[r*6+c] = OnColour; else keyColor12[r*6+c] = Blue; } 
...
////////////////////////////////////////////////////////////////////////////
void ShowPipes()  // This is Pipe Indexes not PipeID's
////////////////////////////////////////////////////////////////////////////
...
{ byte p, n;
  ...
  char SPStr[]   = "S.P  .   .   .   .   .   .   .   .  ";
  p = 4; 
  if (Layout==1) for (n=0; n<8; n++) { if (SlotPipeId1[n]<0x7F) { SPStr[p]=n+49; SPStr[p+2]=SlotPipeId1[n]+48; p=p+4; } } // PipeID PipeIdx1
  if (Layout==2) for (n=0; n<8; n++) { if (SlotPipeId2[n]<0x7F) { SPStr[p]=n+49; SPStr[p+2]=SlotPipeId2[n]+48; p=p+4; } } // PipeID PipeIdx2
  status((char *)SPStr); 
  ConfigButtons(0);  
}
...
{ byte r, c, b;
  ...
  switch(xpoint){                                           // Layout=1 or Layout=2
    ...
    case 11: //////////////////////////////////////////////// Option2 Dwn
      ...
      break; 
     case 17: //////////////////////////////////////////////// Nxt
      Layout++; Routing = SlotOption = Slot = Pipes = false; if (Layout>2) ConfigKeys(1); else ConfigButtons(1); delay(keydelay);
      break;     
    case 23: //////////////////////////////////////////////// Option3      
      Pipes = !Pipes; if (Pipes) ShowPipes(); ConfigButtons(0); delay(keydelay);    
      break;      
    case 29: //////////////////////////////////////////////// Option4 
  ...
  switch(xpoint){
    case 3: 
      ...
      break;
    case 7: 
       Routing = SlotOption = Slot = Pipes = false; Layout++; if (Layout>4) { Layout=1; ConfigButtons(1); } else ConfigKeys(1);
      break;
    case 10: 
