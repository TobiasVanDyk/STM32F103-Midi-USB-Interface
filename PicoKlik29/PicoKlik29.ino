...
/////////////////////////////////////////////////////////
  ...
  // Layouts 1 and 2 = XPoint 5x5 switches only initialised before first blank timeout set here at 15 seconds
...
{ byte b, n, i;
  ...
  ConfigButtons(0);
  //  /*
  n = 7; int m = 0; int a = 15;             // n=4 start              
  while (n<a) { b = SetPipe[n];                                  
                Int2HexStr[n+m]   = digits[(b >> 4) & 0x0F];   
                Int2HexStr[n+m+1] = digits[b & 0x0F]; n++; m=m+3; }    
  status((char *)Int2HexStr); 
  // */
...
{ byte r, c, b;
  ...
  switch(xpoint){                                           // Layout=1 or Layout=2
    ...
    case 11: //////////////////////////////////////////////// Option2 Dwn
     ...
     case 17: //////////////////////////////////////////////// Nxt
      ...
      break;     
    case 23: //////////////////////////////////////////////// Option3      
      Pipes = !Pipes; if (Pipes) ShowPipes(0); ConfigButtons(0); delay(keydelay);    
      break;      
    case 29: //////////////////////////////////////////////// Option4 
  ...
  switch(xpoint){
    case 0: 
      if (Layout==4 && LayerAD==1) { status("Rebooting Pico in 1s"); delay(1000); rp2040.reboot(); }    // C10             
      break;
    case 1: 
      if (Layout==4 && LayerAD==1) { SXChoice = false; ShowPipes(1); return;}                           // C11
      break;  
    case 2: 
      if (Layout==4 && LayerAD==1) { SXChoice = false; ShowSysX = !ShowSysX;                            // C12
      if (ShowSysX) status("Show SysEx Received"); else status("Hide SysEx Received"); return; } 
      break;          
    case 3: 
      if (LayerAD<3) LayerAD++; else LayerAD = 0; delay(keydelay); ConfigKeys(0);  
      break;
    case 4: 
      if (Layout==4 && LayerAD==1) { DoSync = true; Sync = 0; 
      LastMillis = NowMillis = 0; status("Resync Pico MidikLik in 10s"); return; }                      // C13
      break;
    case 5: 
      if (Layout==4 && LayerAD==1) { ShowPipesBypass(1); return; }                                      // C14
      break;             
    case 7: 
    ...
    if (b<100 && Layout==4) { MIDI1.sendSysEx(CCSize, Ctr1to36[b], true); delay(keydelay); return; }     
